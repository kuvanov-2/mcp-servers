...
class JiraFetcher:
    ...
    def __init__(self):
        ...
        # Configure session with proper encoding
    ...
    def get_issue(self, issue_key: str, expand: Optional[str] = None) -> Document:
        ...
        try:
            ...
            # Debug log
            logger.warning(f"Raw issue response: {issue}")
            logger.warning(f"Raw description type: {type(issue['fields'].get('description'))}")
            logger.warning(f"Raw description: {issue['fields'].get('description')}")
